...
export default function PredictionsPage() {
  ...
  const [overlayType, setOverlayType] = useState<'class' | 'instance' | 'none'>('class');
  const [patchStems, setPatchStems] = useState<string[]>([]);
  const [patchOriginals, setPatchOriginals] = useState<(string | null)[]>([]);
  const [patchMasksClass, setPatchMasksClass] = useState<string[]>([]);
  const [patchMasksInstance, setPatchMasksInstance] = useState<(string | null)[]>([]);
  
  // Filter state
  const [filter, setFilter] = useState<"All" | "Brain" | "Heart" | "Cell" | "Unknown">("All");
  ...
  // removed progress polling

  const fetchNiftiFiles = async () => {
    setIsFetchingFiles(true);
    try {
      const response = await fetch("http://localhost:5328/api/inference/jobs", {
        method: "GET",
        credentials: "include",
      });
      ...
      if (!data.success) {
        throw new Error(data.error || "Failed to fetch inference jobs");
      }
      
      // Transform generic inference jobs to the expected format
      const filesWithType = (data.jobs || []).map((job: any) => ({
        id: job.job_id,
        filename: job.filename || `Job ${job.job_id}`,
        jobId: job.job_id,
        scanType: job.model_type === 'nnunet' ? 
                 (job.filename?.includes("BRATS") ? "Brain" as const :
                  job.filename?.includes("_la_") ? "Heart" as const : "Unknown" as const) :
                 job.model_type === 'cellpose' ? "Cell" as const : "Unknown" as const,
      ...
      const cellFiles = sortedFiles.filter((f: NiftiFile) => f.scanType === "Cell").length;
      setBrainCount(brainFiles);
      setHeartCount(heartFiles);
      setCellCount(cellFiles);
    } catch (error) {
  ...
  const handleGalleryOpen = async (niftiId: string, jobId: string) => {
    setIsLoading(true);
    try {
      // Use new patches endpoint for robust viewing
      const resp = await fetch(`${BACKEND_URL}/api/inference/patches/${jobId}`, { credentials: "include" });
      if (!resp.ok) {
        const d = await resp.json();
        throw new Error(d.error || "Failed to fetch patches");
      }
      const d = await resp.json();
      if (!d.success) throw new Error(d.error || "Failed to fetch patches");

      setPatchStems(d.stems || []);
      setPatchOriginals(d.originals || []);
      setPatchMasksClass(d.masks_class || []);
      setPatchMasksInstance(d.masks_instance || []);
      const count = (d.masks_class || []).length;
      setGallerySlices({ original: d.originals || [], result: d.masks_class || [], totalSlices: count });
  ...
  const handleSendToCVAT = async () => {
    ...
    try {
      // Try without credentials first (will work if backend has a session token)
      const resp = await fetch("http://localhost:5328/cvat/upload_tasks", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ nifti_ids: selectedNiftis }),
      });
      if (resp.status === 401) {
        // Ask for credentials only if required
      ...
      const data = await resp.json();
      if (!data.success) throw new Error(data.error || "Failed to send to CVAT");
      toast.success(`Successfully sent ${selectedNiftis.length} task(s) to CVAT`);
      setSelectedNiftis([]);
    } catch (e) {
      console.error(e);
      toast.error(e instanceof Error ? e.message : "Failed to send to CVAT");
    } finally {
  ...
  const handleDiscardFiles = async () => {
    ...
    setIsProcessing(true);
    try {
      // Use generic inference delete endpoint per job id
      const results = await Promise.all(
        selectedNiftis.map(async (jobId) => {
          const resp = await fetch(`${BACKEND_URL}/api/inference/delete/${jobId}`, {
            method: "DELETE",
            credentials: "include",
          });
          if (!resp.ok) {
            try { const d = await resp.json(); throw new Error(d.error || 'Delete failed'); } catch { throw new Error('Delete failed'); }
          }
          const d = await resp.json();
  ...
  const handleDownloadResults = async (jobId: string) => {
    try {
      const response = await fetch(`http://localhost:5328/api/inference/download/${jobId}`, {
        method: "GET",
        credentials: "include",
      });
      ...
      if (!response.ok) {
        if (response.status === 400) {
          const data = await response.json();
          throw new Error(data.error || "Job is not completed yet");
        }
      ...
      a.href = url;
      a.download = `inference_results_${jobId}.zip`;
          ...
          <div className="bg-white shadow-xl rounded-xl p-6 border border-gray-200">
            ...
            <div className="flex justify-center mb-6">
              <div className="inline-flex rounded-md shadow-sm" role="group">
                {["All", "Brain", "Heart", "Cell", "Unknown"].map((type) => (
                  <button
                    key={type}
                    type="button"
                    onClick={() => setFilter(type as "All" | "Brain" | "Heart" | "Cell" | "Unknown")}
                  ...
                  <tbody>
                    {filteredFiles.map((file) => (
                      <tr key={file.id} className="border-t">
                        ...
                        {/* <td className="p-4 text-gray-600">date hidden</td> */}
                        <td className="p-4">
                          <div className="flex space-x-2">
                            ...
                            <button
                              onClick={() => handleDownloadResults(file.jobId)}
          ...
          <div className="mt-6 pt-4 border-t border-gray-200">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="p-3 bg-green-50 rounded-lg">
                <p className="text-xs text-green-600">Cell Images</p>
          ...
          <div className="bg-white rounded-lg shadow-xl relative max-w-6xl w-full flex flex-col h-[90vh]">
            ...
            <div className="flex flex-1 overflow-hidden">
              ...
              <div className="flex-1 relative">
                ...
                {/* Result image overlay: class or instance mask */}
                {overlayType !== 'none' && (
                  (() => {
                    const src = overlayType === 'class' ? patchMasksClass[currentSliceIndex] : patchMasksInstance[currentSliceIndex];
                    if (!src) return null;
                    return (
                      <img
                        src={src}
                        alt={`${overlayType} mask ${currentSliceIndex + 1}`}
                        className="absolute inset-0 w-full h-full object-contain z-[2]"
                        style={{ opacity: overlayOpacity, mixBlendMode: useScreenBlend ? 'screen' : 'normal' }}
                      />
                    );
                  })()
                )}
            ...
            <div className="p-4 border-t border-gray-200 bg-gray-50">
              <div className="flex items-center justify-between space-x-4">
                ...
                <div className="flex-1">
                  <input
                    type="range"
                    min={0}
                    max={Math.max(0, patchMasksClass.length - 1)}
                  ...
                  <div className="flex justify-between text-sm text-gray-500 mt-1">
                    <span>Slice 1</span>
                    <span>Slice {patchMasksClass.length || 0}</span>
                  </div>
                </div>
                
                <button
                  onClick={() => navigateSlice('next')}
                  disabled={currentSliceIndex === Math.max(0, patchMasksClass.length - 1)}
                  className="px-4 py-2 bg-blue-600 text-white rounded-lg disabled:opacity-50"
                  type="button"
                >
                  Next
              ...
              {/* Quick jump buttons (slice navigation) */}
              {patchMasksClass.length > 1 && (
                <div className="flex justify-center mt-4 space-x-2">
                  {[0, 0.25, 0.5, 0.75, 1].map((fraction) => {
                    const total = patchMasksClass.length;
                    const sliceIndex = Math.floor(fraction * (total - 1));
                    return (
                      <button
                        key={fraction}
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                <div className="flex items-center space-x-2">
                  ...
                  <button type="button" onClick={() => setOverlayType('none')} className={`px-2 py-1 rounded text-sm ${overlayType==='none'?'bg-gray-800 text-white':'bg-gray-200 text-gray-800'}`}>None</button>
                  <button type="button" onClick={() => setOverlayType('class')} className={`px-2 py-1 rounded text-sm ${overlayType==='class'?'bg-blue-600 text-white':'bg-gray-200 text-gray-800'}`}>Class</button>
                  <button type="button" onClick={() => setOverlayType('instance')} className={`px-2 py-1 rounded text-sm ${overlayType==='instance'?'bg-purple-600 text-white':'bg-gray-200 text-gray-800'}`}>Instance</button>
                </div>
                <div className="flex items-center space-x-2">
                  <span className="text-sm text-gray-600">Blend:</span>
                  <button type="button" onClick={() => setUseScreenBlend(false)} className={`px-2 py-1 rounded text-sm ${!useScreenBlend?'bg-gray-800 text-white':'bg-gray-200 text-gray-800'}`}>Normal</button>
                  <button type="button" onClick={() => setUseScreenBlend(true)} className={`px-2 py-1 rounded text-sm ${useScreenBlend?'bg-gray-800 text-white':'bg-gray-200 text-gray-800'}`}>Screen</button>
                </div>
                <div className="flex items-center space-x-3">
                  <span className="text-sm text-gray-600 w-16">Opacity</span>
                  <input type="range" min={0} max={1} step={0.05} value={overlayOpacity} onChange={(e) => setOverlayOpacity(parseFloat(e.target.value))} className="flex-1" />
                  <span className="text-sm text-gray-600 w-10 text-right">{Math.round(overlayOpacity*100)}%</span>
                </div>
              </div>
            </div>
          </div>
