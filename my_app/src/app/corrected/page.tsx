...
type CorrectedTask = {
  ...
  scanType: "Brain" | "Heart" | "Cell" | "Unknown";
  modelType: "nnunet" | "cellpose" | "unknown";
};
...
function getDatasetType(filename: string): "Brain" | "Heart" | "Cell" | "Unknown" {
  if (filename.includes("BRATS")) return "Brain";
  if (filename.includes("la")) return "Heart";
  if (filename.includes("cell") || filename.includes("nucleus")) return "Cell";
  return "Unknown";
}

function getModelType(filename: string): "nnunet" | "cellpose" | "unknown" {
  if (filename.includes("BRATS") || filename.includes("la_")) return "nnunet";
  if (filename.includes("cell") || filename.includes("nucleus")) return "cellpose";
  return "unknown";
}
...
export default function CorrectedPage() {
  ...
  const [filter, setFilter] = useState<"All" | "Brain" | "Heart" | "Cell">("All");
  const [modelFilter, setModelFilter] = useState<"All" | "nnunet" | "cellpose">("All");
  ...
  const fetchCorrectedTasks = async () => {
    try {
      ...
      const tasks: CorrectedTask[] = data.correctedTasks.map((t: any) => {
        ...
        const modelType = getModelType(raw);
        return {
          task_id: t.task_id,
          task_name: raw,
  ...
  // filter by scanType and modelType
  const filteredTasks = correctedTasks.filter((t) => {
    const scanTypeMatch = filter === "All" ? true : t.scanType === filter;
    const modelTypeMatch = modelFilter === "All" ? true : t.modelType === modelFilter;
    return scanTypeMatch && modelTypeMatch;
  });

  const handleFilterChange = (f: "All" | "Brain" | "Heart" | "Cell") => {
    setFilter(f);
  };

  const handleModelFilterChange = (f: "All" | "nnunet" | "cellpose") => {
    setModelFilter(f);
  };
          ...
          <div className="flex flex-col md:flex-row md:justify-between items-center mb-8 gap-4">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="inline-flex rounded-md shadow-sm">
                <span className="px-3 py-2 text-sm font-medium text-gray-700 bg-gray-100 border border-gray-200 rounded-l-lg">
                  Dataset:
                </span>
                {(["All", "Brain", "Heart", "Cell"] as const).map((type) => (
                  <button
                    key={type}
              ...
              <div className="inline-flex rounded-md shadow-sm">
                ...
                {(["All", "nnunet", "cellpose"] as const).map((type) => (
                  <button
                    key={type}
                    onClick={() => handleModelFilterChange(type)}
                    className={`px-4 py-2 text-sm font-medium border ${
                      modelFilter === type
                      ...
                      <tr key={t.task_id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-sm text-gray-900">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            t.scanType === 'Brain' ? 'bg-blue-100 text-blue-800' :
                            t.scanType === 'Heart' ? 'bg-pink-100 text-pink-800' :
                            t.scanType === 'Cell' ? 'bg-green-100 text-green-800' :
                        ...
                        <td className="px-6 py-4 text-sm text-gray-900">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            t.modelType === 'nnunet' ? 'bg-purple-100 text-purple-800' :
                            t.modelType === 'cellpose' ? 'bg-orange-100 text-orange-800' :
                            'bg-gray-100 text-gray-800'
                          }`}>
                            {t.modelType.toUpperCase()}
                          </span>
                        </td>
                      </tr>
                    ))}
